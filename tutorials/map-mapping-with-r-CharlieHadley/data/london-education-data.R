...
download.file("https://data.london.gov.uk/download/age-when-completed-continuous-full-time-education-borough/02bd7908-d260-4dba-982e-4eaec216e4e0/age-when-completed-continuous-full-time-education.xls",
              "data/age-when-completed-full-time-education.xls")

raw_london_age_finished_edu <- read_xls("data/age-when-completed-full-time-education.xls",
                                        sheet = "2018", skip = 1)
...
# remove the "Never had an education" group as they only appear in one borough
london_age_finished_edu <- london_age_finished_edu %>%
  filter(!age_group == "Never had an education") 

london_age_finished_edu %>% 
  write_csv("data/age-when-completed-education.csv")

unlink("data/age-when-completed-full-time-education.xls")
